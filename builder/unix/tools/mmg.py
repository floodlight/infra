...
class ManifestBase(object):
    ...
    def generate(self, directories=['.'], output=None):
...
""" % dict(DESCRIPTION=self.desc, TIMESTAMP=self.now)
        ...
        target = output if output else self.target
...
if __name__ == "__main__":
    ...
    ap.add_argument("classes", choices = classes.keys() + [ 'all' ], nargs='+')
    ap.add_argument("--out")
    ap.add_argument("--only-if-missing", action='store_true')

    ops = ap.parse_args()

    if 'all' in ops.classes:
        ops.classes = classes.keys()
    else:
        ops.classes = list(set(ops.classes))

    if os.path.exists(ops.out) and ops.only_if_missing:
        pass
    else:
        for name in ops.classes:
            x = classes[name]()
            x.generate(ops.dirs, ops.out)

    if ops.out:
        print os.path.abspath(ops.out)
